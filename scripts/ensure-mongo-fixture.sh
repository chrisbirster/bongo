...
container_current() {
    ...
    [[ "$(container_label "$name" "io.bongo.fixture")" == "$fixture" ]] ||
        return 1

    [[ "$(container_label "$name" "io.bongo.fixture-version")" == "$FIXTURE_VERSION" ]] ||
        return 1

    [[ "$(container_image_id "$name")" == "$(expected_image_id)" ]] ||
        return 1
...
ensure_standalone() {
    if container_current mongodb standalone &&
       standalone_ready >/dev/null 2>&1; then
...
ensure_replica() {
    if container_current mongodb-rs replica &&
       replica_ready >/dev/null 2>&1; then
...
tls_cert_current() {
    ...
    # Consider the fixture stale if the certificate expires within an hour.
    openssl x509 \
        -checkend 3600 \
        -noout \
        -in "$cert" \
...
generate_tls_certificate() {
    ...
    # Suppress OpenSSL's progress-noise. Errors still cause the script to fail.
    openssl req \
        -x509 \
        -newkey rsa:2048 \
        -nodes \
...
case "${1:-}" in
    ...
    clean)
        ...
        ;;
    *)
        echo "usage: $0 {standalone|tls|replica|status|reset|clean}" >&2
        exit 2
        ;;
esac
