...
pub const Client = struct {
    ...
    pub fn findOneAndUpdate(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeFindAndModifyUpdate(
                self.allocator,
                request_id,
                database_name,
                collection_name,
            ...
            try find_and_modify.encodeUpdate(
                self.allocator,
                request_id,
                database_name,
                collection_name,
    ...
    pub fn findOneAndReplace(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeFindAndModifyReplace(
                self.allocator,
                request_id,
                database_name,
                collection_name,
            ...
            try find_and_modify.encodeReplace(
                self.allocator,
                request_id,
                database_name,
                collection_name,
    ...
    pub fn findOneAndDelete(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeFindAndModifyDelete(
                self.allocator,
                request_id,
                database_name,
                collection_name,
            ...
            try find_and_modify.encodeDelete(
                self.allocator,
                request_id,
                database_name,
                collection_name,
    ...
    pub fn insertOne(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeInsertOne(
                self.allocator,
                request_id,
                database_name,
                collection_name,
    ...
    pub fn insertMany(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeInsert(
                self.allocator,
                request_id,
                database_name,
                collection_name,
    ...
    pub fn replaceOneWithOptions(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeReplaceOne(
                self.allocator,
                request_id,
                database_name,
                collection_name,
            ...
            try replacement_ops.encodeReplaceOneWithOptions(
                self.allocator,
                request_id,
                database_name,
                collection_name,
    ...
    fn runUpdate(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeUpdate(
                self.allocator,
                request_id,
                database_name,
                collection_name,
    ...
    fn delete(
        ...
        const request_id = self.takeRequestId();
        const request = if (self.write_concern) |concern|
            try write_command.encodeDelete(
                self.allocator,
                request_id,
                database_name,
                collection_name,
