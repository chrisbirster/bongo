...
/// credential. The handshake requests mechanism support and speculatively
/// starts SCRAM-SHA-256 in the same round trip.
pub fn authenticate(
...
/// Encode the initial hello command used for SCRAM mechanism negotiation.
pub fn encodeRequest(
    ...
    const principal = try std.fmt.allocPrint(
        allocator,
        "{s}.{s}",
        .{ database, username },
    );
...
fn selectMechanism(body: []const u8) !sasl.Mechanism {
    const value = (try bson.Reader.get(body, "saslSupportedMechs")) orelse
        return .scram_sha_1;

    const array = switch (value) {
        .array => |bytes| bytes,
        // The auth specification says drivers must not reject unknown or
        // unexpected mechanism contents. Fall back to SHA-1 when negotiation
        // is unusable, matching the missing-field behavior.
    ...
    while (try reader.next()) |element| {
        switch (element.value) {
            .string => |name| {
                if (std.mem.eql(u8, name, "SCRAM-SHA-256")) {
...
fn speculativeResponse(
    allocator: Allocator,
    body: []const u8,
) !?sasl.Response {
    const value = (try bson.Reader.get(body, "speculativeAuthenticate")) orelse
        return null;
...
test "auth handshake requests mechanisms and speculative SHA-256" {
    const bytes = try encodeRequest(
    ...
    try std.testing.expectEqualStrings(
        "admin.alice",
        (try bson.Reader.get(body, "saslSupportedMechs")).?.string,
    );

    const speculative =
        (try bson.Reader.get(body, "speculativeAuthenticate")).?.document;
    try std.testing.expectEqualStrings(
        "SCRAM-SHA-256",
        (try bson.Reader.get(speculative, "mechanism")).?.string,
    );
    try std.testing.expectEqualStrings(
        "admin",
        (try bson.Reader.get(speculative, "db")).?.string,
    );
}
...
test "mechanism selection prefers SHA-256" {
    ...
    try std.testing.expectEqual(
        sasl.Mechanism.scram_sha_256,
        try selectMechanism(body),
    );
}
...
test "mechanism selection falls back to SHA-1" {
    ...
    try std.testing.expectEqual(
        sasl.Mechanism.scram_sha_1,
        try selectMechanism(missing),
    );

    const unknown_only = try bson.encode(
        std.testing.allocator,
        .{ .saslSupportedMechs = [_][]const u8{"UNKNOWN"} },
    );
    defer std.testing.allocator.free(unknown_only);
    try std.testing.expectEqual(
        sasl.Mechanism.scram_sha_1,
        try selectMechanism(unknown_only),
    );
}
